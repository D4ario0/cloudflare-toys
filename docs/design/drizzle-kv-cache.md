...
`cloudflareKVCache(binding, options)` returns a `CloudflareKVCache`, which extends Drizzle's public `Cache` class. Options cover a namespace prefix, a default TTL, Drizzle's `explicit` or `all` strategy, and an error observer. The KV binding is structurally typed to the three methods used by the adapter.
...
Automatic table invalidation is disabled. Cached queries with automatic invalidation enabled are rejected. Table-only mutation notifications are ignored so ordinary mutations remain usable with manually invalidated caches. No table dependency data is written.
...
Reads and writes fail open and report through `onError` or `console.error`. A failed cache must not prevent a source query from succeeding. Malformed data is a reported miss. Invalidation fails closed because it is requested after the caller knows cached data is stale. Configuration errors fail closed because silently ignoring TTL or invalidation semantics is unsafe.
